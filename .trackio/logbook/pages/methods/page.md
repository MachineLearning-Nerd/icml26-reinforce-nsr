...
---
<!-- trackio-cell
{"type": "markdown", "id": "cell_e8f992a60951", "created_at": "2026-07-20T07:35:55+00:00", "title": "Independent routes"}
-->
# Methods

The first route expands the paper\x27s one-step theorem with an independent Wick implementation. The second evaluates the trajectory estimator by product Gauss-Hermite quadrature; it also obtains the objective through a covariance recursion and differentiates it independently. The nonlinear route evaluates each conditional moment directly by quadrature.


---
<!-- trackio-cell
{"type": "code", "id": "cell_fcdd82bd8a7c", "created_at": "2026-07-20T07:36:20+00:00", "title": "Run: python verify_reinforce_nsr.py (exit 0)", "command": ["python", "repro/src/verify_reinforce_nsr.py", "--output", "outputs/independent_verification.json"], "exit_code": 0, "duration_s": 17.2}
-->
````bash
$ python repro/src/verify_reinforce_nsr.py --output outputs/independent_verification.json
````

exit 0 · 17.2s


````python title=verify_reinforce_nsr.py
#!/usr/bin/env python3
  ...
  "claims": [
    {
      ...
      "one_step": {
        "cells": 12,
        "max_mean_abs_error": 9.71445146547012e-15,
        "max_second_relative_error": 1.0945388219942057e-15,
        "order4_controls_rejected": 12,
        "order4_max_relative_drift": 0.2772126100936175
      },
      "finite_horizon": {
        "cells": 13,
        "max_gradient_abs_error": 6.092637505616949e-10,
        "max_gradient_relative_error": 1.699413254125038e-10,
        "max_return_abs_error": 9.325873406851315e-15,
        "minimum_variance": 0.4993175414338385
      }
    },
    {
      "id": "C2",
      ...
      "landscape": {
        "cells": 3721,
        "minimum_nsr": 18.500460007174897,
        "maximum_nsr": 4037626453.4742846,
        "span_ratio": 218244651.8577593,
        "small_sigma_slope_vs_std": -1.999999369614449,
        "monotone_blowup_failures": 0
      }
    },
    {
      "id": "C3",
      ...
      "nonlinear_bound": {
        "cells": 48,
        "theta_bound_failures": 0,
        "logstd_bound_failures": 0,
        "min_theta_ratio": 3.1576817455575643,
        "min_logstd_ratio": 4.871985172717222,
        "no_inverse_mutation_failures": 26
      }
    }
  ]
}
...
---
<!-- trackio-cell
{"type": "code", "id": "cell_1247fe926c21", "created_at": "2026-07-20T07:36:21+00:00", "title": "Run: python (exit 0)", "command": ["python", "-m", "pytest", "-q", "repro/tests"], "exit_code": 0, "duration_s": 0.442}
-->
````bash
